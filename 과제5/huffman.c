...
int main( int argc, char **argv)
{
	...
	// 문자별 빈도 출력 (for debugging) 나중에 다시 주석 표시해서 제출
...
tNode* make_huffman_tree(int* ch_freq) {
	...
	int capacity = 256;
	HEAP*  heapPtr = heapCreate(capacity); //1번

	for (int i = 0; i < 256; i++) {
		tNode* nodePtr = newNode(i, ch_freq[i]);
		int check = heapInsert(heapPtr, nodePtr);
		if (!check) printf("heap error: full heap");
	} //2, 3번

	do {
		tNode* data1 = heapDelete(heapPtr);
		tNode* data2 = heapDelete(heapPtr); //4번
		int freqSum = (data1->freq) + (data2->freq);
		tNode* newData = newNode(0, freqSum);
		newData->left = data1;
		newData->right = data2; //5번
		int check = heapInsert(heapPtr, newData);
		if (!check) printf("heap error: full heap"); //6번
	} while ((heapPtr->last) > 0); //7번

	tNode* rootNode = heapDelete(heapPtr);
	heapDestroy(heapPtr);
...
void traverse_tree(tNode* root, char* code, int depth, char* codes[]) {
	...
	if (((root->left) == NULL) && ((root->right) == NULL)) {
		code[depth] = '\0';
		int dataNumber = root->data;
		codes[dataNumber] = strdup(code);
		return;
	}
